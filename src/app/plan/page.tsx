...
import { getOptimalWindows, getStockList, getTimeOfDayData, getSessionPhases, getMarketRegime } from "@/lib/mock-data";
...
export default function PlanPage() {
  ...
  const windows = getOptimalWindows(watchlist);
      ...
      <div className="bg-vortex-card border border-vortex-border rounded-xl p-5 mb-6">
        ...
        <div className="flex gap-4 text-[10px]">
          <span className="text-vortex-muted">
            VIX: <span className="text-vortex-text font-mono">{regime.vixLevel}</span>
          </span>
          <span className="text-vortex-muted">
            Trend WR:{" "}
          ...
          <>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {windows.map(({ ticker, windows: w }) => {
                  const todData = getTimeOfDayData(ticker);
                  const phases = getSessionPhases(ticker);
