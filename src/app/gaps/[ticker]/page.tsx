...
import { getGapFillData, getStockMeta } from "@/lib/mock-data";
import TickerSearch from "@/components/TickerSearch";
import StatCard from "@/components/StatCard";

export default function GapFillPage({ params }: { params: Promise<{ ticker: string }> }) {
  const { ticker } = use(params);
  const upperTicker = ticker.toUpperCase();
  const data = getGapFillData(upperTicker);
  ...
  const avgUpFill = upGaps.reduce((sum, d) => sum + d.fillRate, 0) / upGaps.length;
  const avgDownFill = downGaps.reduce((sum, d) => sum + d.fillRate, 0) / downGaps.length;
  const totalSamples = data.reduce((sum, d) => sum + d.sampleSize, 0);
  const bestEdge = data.reduce((best, d) => (d.fillRate > best.fillRate ? d : best), data[0]);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <TickerSearch currentTicker={upperTicker} basePath="/gaps" />
      </div>
