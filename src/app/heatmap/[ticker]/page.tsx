...
import { getTimeOfDayData, getStockMeta } from "@/lib/mock-data";
...
export default function HeatmapPage({ params }: { params: Promise<{ ticker: string }> }) {
  const { ticker } = use(params);
  const upperTicker = ticker.toUpperCase();
  const data = getTimeOfDayData(upperTicker);
  const stock = getStockMeta(upperTicker);

  const maxVol = Math.max(...data.map((d) => d.avgVolume));
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <TickerSearch currentTicker={upperTicker} basePath="/heatmap" />
      </div>
        ...
        <div className="grid grid-cols-[100px_repeat(13,1fr)] gap-1 mb-1">
          ...
          {data.map((d) => {
            ...
            return (
              ...
              >
                <span className="text-[9px] font-mono text-white/80">{d.volatility}%</span>
              </div>
            );
          })}
        </div>
            ...
            <tbody>
              {data.map((d) => (
                <tr key={d.time} className="border-b border-vortex-border/50 hover:bg-vortex-surface/50">
                  ...
                  <td className="py-2 text-right font-mono text-vortex-text">{d.volatility}%</td>
